...
// TODO may print log for MB

void Alp::ProcessMessage(const MbReadResponseMsg *pMbReadResponseMsg) {
  unsigned long agent_id = pMbReadResponseMsg->GetOriginalAgent().GetId();
...
void Alp::ProcessMessage(const MbWriteResponseMsg *pMbWriteResponseMsg) {
  unsigned long agent_id = pMbWriteResponseMsg->GetOriginalAgent().GetId();
...
int Alp::GetNumOfAttachedAgents() {
  return this->managed_agents_.size();
}
