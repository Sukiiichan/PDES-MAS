...
unsigned long StateVariable::GetReadUntil() const {
   return fReadUntil;
}
...
AbstractValue* StateVariable::ReadMb(const LpId& pReadingAgent, unsigned long pTime, unsigned long pReadUntil){
   SerialisableList<WritePeriod>::reverse_iterator reverseWritePeriodIterator = fWritePeriodList.rbegin();
   while (reverseWritePeriodIterator != fWritePeriodList.rend()){
      if(reverseWritePeriodIterator->GetStartTime() ){
