      ...
      case WRITEMESSAGE: {
       ...
       case MAILBOXREADMESSAGE:{
         MailboxReadMessage* mailboxReadMessage = static_cast<MailboxReadMessage*>(*iter);
          if (mailboxReadMessage->GetOriginalAgent() == pOriginalAlp && mailboxReadMessage->GetTimestamp() >= pTime) {
       ...
       case MAILBOXWRITEMESSAGE:{
          ...
          if (mailboxWriteMessage->GetOriginalAgent() == pOriginalAlp && mailboxWriteMessage->GetTimestamp() > pTime) {
